...
export default function YangiShartnoma() {
  ...
  const inp = 'w-full bg-[#0B1220] border border-[#1E293B] text-gray-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-blue-600 focus:ring-1 focus:ring-blue-600/20 placeholder-gray-500 transition'
  const inp2 = 'w-full bg-[#0B1220] border border-[#1E293B] text-gray-200 rounded px-2 py-1.5 text-sm focus:outline-none focus:border-blue-600 placeholder-gray-500'

  // ── Render ────────────────────────────────────────────────────────────────────

  return (
    <div className="min-h-full bg-[#060B14]">

      {/* ── Page Header ── */}
      <div className="sticky top-0 z-40 bg-[#060B14]/95 backdrop-blur border-b border-[#1E293B]">
        <div className="max-w-6xl mx-auto px-6 py-3 flex items-center justify-between gap-4">
          {/* Left: back + title */}
          <div className="flex items-center gap-3 min-w-0">
            <button type="button" onClick={() => router.push('/dashboard/shartnomalar')}
              className="flex items-center gap-1.5 text-gray-400 hover:text-white transition text-sm flex-shrink-0">
            ...
            <div className="flex items-center gap-2 min-w-0">
              <div className="w-1 h-5 rounded-full flex-shrink-0" style={{ backgroundColor: selectedTypeMeta?.color || '#3B82F6' }}/>
              <h1 className="text-sm font-semibold text-white truncate">Yangi shartnoma</h1>
              <span className="text-xs text-gray-500 hidden sm:inline truncate">
                — {(CONTRACT_TYPE_NAMES as Record<string, string>)[form.contract_type]}
              </span>
            </div>
          </div>
            ...
            {STEPS.map((s, i) => (
              <div key={i} className="flex items-center">
                <button type="button"
                  ...
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold transition flex-shrink-0 ${
                    step === i + 1 ? 'bg-blue-600 text-white' : step > i + 1 ? 'bg-[#1E293B] text-blue-400' : 'bg-[#1E293B] text-gray-500'
                  }`}>
                    {step > i + 1
                      ? <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7"/></svg>
                      : i + 1}
                  </div>
                  <span className={`text-[11px] font-medium hidden lg:block ${step === i + 1 ? 'text-white' : step > i + 1 ? 'text-gray-400' : 'text-gray-600'}`}>{s}</span>
                </button>
                {i < STEPS.length - 1 && <div className={`w-6 h-px mx-1 ${step > i + 1 ? 'bg-blue-600/40' : 'bg-[#1E293B]'}`}/>}
              </div>
            ))}
          </div>
        </div>
      ...
      <form onSubmit={handleSave}>
        <div className="max-w-6xl mx-auto px-6 py-8">
          ...
          {step === 1 && (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="space-y-5">
                <div className="bg-[#0D1526] border border-[#1E293B] rounded-xl p-5">
          ...
          {step === 2 && (
            <div className="max-w-2xl mx-auto">
              <div className="bg-[#0D1526] border border-[#1E293B] rounded-xl p-6 space-y-5">
                <div>
          ...
          {step === 3 && (
            <div className="max-w-3xl mx-auto space-y-4">
              ...
              {getStructureForEdit().bolimlar.map((bolim, bi) => (
                <div key={bi} className="bg-[#0D1526] rounded-xl p-4 space-y-3 border border-[#1E293B]">
                  <div className="flex items-center gap-2">
              ...
              {getStructureForEdit().bolimlar.length === 0 && (
                <div className="text-center py-12 text-gray-500 text-sm bg-[#0D1526] rounded-xl border border-[#1E293B]">
                  <p>Bo&apos;limlar yo&apos;q</p>
          ...
          {step === 4 && (
            <div className="space-y-4">
              ...
              {form.spec_items.length > 0 ? (
                <div className="bg-[#0D1526] border border-[#1E293B] rounded-xl overflow-hidden">
                ...
                <div className="text-center py-16 text-gray-500 text-sm bg-[#0D1526] rounded-xl border border-[#1E293B]">
              ...
              {form.spec_items.length > 0 && (
                <div className="flex justify-end">
                  <div className="bg-[#0D1526] border border-[#1E293B] rounded-xl p-4 min-w-64 space-y-2">
        ...
        {/* ── Footer ── */}
        <div className="sticky bottom-0 z-40 bg-[#060B14]/95 backdrop-blur border-t border-[#1E293B]">
