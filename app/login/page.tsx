...
import { FileText, ShieldCheck, Users, Bot, Eye, EyeOff } from 'lucide-react'

type LoginTab = 'eimzo' | 'email'

const FEATURES = [
  { icon: FileText,    title: 'Avtomatik shartnomalar',  desc: "STIR bo'yicha rekvizitlarni avtomatik to'ldirish" },
  { icon: ShieldCheck, title: 'E-imzo integratsiyasi',   desc: 'Elektron imzo bilan xavfsiz hujjat imzolash' },
  { icon: Users,       title: "Ko'p tashkilot",          desc: "Bir hisobda bir nechta tashkilotni boshqarish" },
  { icon: Bot,         title: 'AI yordamchi',            desc: 'Hujjatlarni AI yordamida yarating va tahlil qiling' },
]
          ...
          {tab === 'eimzo' && (
            <div className="space-y-3">
                  ...
                  {certs.length > 0 ? (
                    <>
                      ...
                      <button onClick={handleDesktopLogin} disabled={eimzoLoading || !selectedCert}
                        className="w-full flex items-center justify-center gap-2 bg-gradient-to-r from-blue-600 to-blue-500 hover:opacity-90 disabled:opacity-50 text-white font-semibold py-3 rounded-xl transition text-sm">
                        {eimzoLoading
              ...
              <div className="flex items-center gap-3 my-1">
                ...
                <span className="text-gray-400 text-xs">yoki</span>
                <div className="flex-1 h-px bg-gray-100"/>
              </div>

              <button onClick={() => { setTab('email'); setError('') }}
                className="w-full border border-gray-200 py-3 rounded-xl hover:bg-gray-50 transition text-sm text-gray-700 font-medium">
                ✉ Email orqali kirish
              </button>
            </div>
          )}
          ...
          {tab === 'email' && (
            <div className="space-y-3">
              ...
              <form onSubmit={handleEmailLogin} className="space-y-3">
                <div className="relative">
                  ...
                  <input type="email" value={email} onChange={e => setEmail(e.target.value)} required
                    className={`${inp} pl-10`} placeholder="Email manzilingiz" />
                </div>
                <div className="relative">
                  <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-400">🔒</span>
                  <input type={showPass ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} required
                    className={`${inp} pl-10 pr-10`} placeholder="Parolingiz" />
                ...
                <div className="flex items-center justify-between text-sm">
                  ...
                  <a href="#" className="text-blue-600 text-xs hover:underline">Parolni unutdingizmi?</a>
                </div>
                <button type="submit" disabled={loading}
                  className="w-full bg-gradient-to-r from-blue-600 to-blue-500 hover:opacity-90 disabled:opacity-50 text-white font-semibold py-3 rounded-xl transition text-sm">
                  {loading ? T(t.btn.loading) : 'Kirish →'}
                </button>
              </form>
              ...
              <button onClick={() => { setTab('eimzo'); setError('') }}
                className="w-full border border-gray-200 py-3 rounded-xl hover:bg-gray-50 transition text-sm text-gray-700 font-medium flex items-center justify-center gap-2">
      ...
      <div className="hidden lg:flex flex-1 relative overflow-hidden text-white">
        ...
        <div className="relative z-10 flex flex-col justify-between p-12 w-full">
          ...
          <div className="flex justify-center relative my-2">
            <div className="absolute w-72 h-40 bg-blue-500 opacity-20 blur-3xl rounded-full top-1/2 -translate-y-1/2"/>
            <img
              src="/laptop-mockup.svg"
              alt="Dashboard"
              className="w-[420px] relative drop-shadow-2xl"
              draggable={false}
            />
          </div>
          ...
          <div className="flex items-center gap-4 bg-black/30 border border-white/15 p-4 rounded-2xl backdrop-blur-md">
            ...
            <div>
              ...
              <div className="text-gray-400 text-xs">O'zbekiston bo'ylab ishonch bilan ishlatilmoqda</div>
            </div>
          </div>
